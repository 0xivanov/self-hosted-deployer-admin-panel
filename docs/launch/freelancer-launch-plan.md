...
- Registry image reference first: Docker Hub/GHCR, port, health check, environment variables, private credential handling, digest pinning and ARM64 compatibility validation before acceptance. Implemented the metadata resolver/check command with bounded requests, digest verification and ARM64 selection; real public metadata probes succeeded for both registries. Local schema 43 also adds disabled-by-default container projects and immutable release storage, with permission rechecks, shared capacity admission and retry-safe digest pins. Local schema 44 adds the deployment queue, cancellation, persisted dispatch intent and authorization rechecks; fleet configuration rendering supports selected ports and health paths. Runtime submission and reconciliation now have a local, explicitly gated worker path with durable revision fences and actual configuration/HTTPS readiness checks. The local portal now provides gated image checking, release history, publishing/restoring and cancellation controls. Browser verification covered a real public metadata check, setup availability refresh, queued publication and cancellation in a disposable database with no fleet worker. Container provisioning, project cleanup and custom-domain port handling are now integrated locally with focused controller checks. Known preflight/local rejection now becomes retryable promptly through a durable no-submission record, with previous-release health and restart replay protection. Ambiguous dispatched failures still require stronger recovery evidence. Core private credential storage and Kubernetes pull-secret support are implemented locally with immutable revisions and previous-credential rollback. Portal credential controls, schema-45 encrypted storage, project-scoped metadata checks and fleet/CLI credential submission are implemented locally. Synthetic browser and focused API/worker checks passed. Environment settings are now implemented locally with schema-46 encrypted versions, portal controls, release references, preflight staging and core immutable Kubernetes Secrets. Local browser, API, worker and rollback checks passed. Confirmed failed-apply withdrawal is now integrated locally: exact receipt/preflight validation, durable no-replay state and fresh predecessor health checks make positively rolled-back failures retryable. Durable request IDs and read-only outcome lookup are implemented locally for lost replies, with exact configuration/identity checks and fresh runtime readiness before settlement. Core now has isolated candidate activation, durable runtime checkpoints, traffic recovery, retirement, selected-release status and atomic request/app/route finalization locally. The internal completion coordinator checks readiness and restores routes before saving outcomes. Public advance/recovery operations and tracked candidate submission are integrated locally. The fleet worker now has gated automatic advancement and durable recovery decisions with fresh authorization checks. Pre-checkpoint recovery and safe retries after initial withdrawal are implemented locally. Candidate-aware project deletion is also implemented locally. Retirement of older releases after successful updates is implemented locally. Missing-request ambiguity, cross-resource late-write qualification and controlled enablement remain unfinished. Credential/version garbage collection and real fleet qualification remain incomplete. This is not deployed or yet a customer deployment feature. Remaining work and evidence are in [registry image deployment](registry-image-deployment.md).
