...
- GitHub connection and deploy-on-push: implementation started with bounded signed push validation and repository archive preparation using the existing static/Node upload rules. Installation linking, durable import/replay handling, exact-commit fetching, queue integration and customer controls remain unfinished; no webhook or GitHub deployment is enabled in production. See [GitHub deployment](github-deployment.md).
