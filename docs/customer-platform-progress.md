...
## Workspace usability refresh, 2026-09-16

Reviewed the signed-in portal in Brave and deployed a Launchstead visual refresh. Projects, Billing and Team now have separate navigation; unavailable domain/store sections and unauthorized controls remain hidden. Project creation is expandable and draft inputs survive navigation. Per-project hosting setup messages explain upload versus publishing availability.
...
## September 23: website portfolio and dedicated management

The portfolio now displays compact website cards with the current publishing workflow status, the published hostname when available, and a Manage website button. Management opens in place with publishing/domain/settings shortcuts, a back-to-list control and support contact. This is an in-page detail view, not a separately bookmarkable route. Filters and existing forms remain mounted. Website section controls are preserved by refreshProject and excluded from release-history organization. Switching workspace or losing access resets selection; deleted selections return to the portfolio on a completed list reload. Header spacing was reduced for laptop screens.

Focused browser verification used only disposable local data: creation, portfolio/detail/back navigation, domain shortcut, preservation of an unfinished hostname through manual status refresh, and light/dark presentation. Existing 22 frontend checks passed, as did Node progress/HTTP tenant-boundary checks and the Linux build. This release includes the previously committed safe build-phase messages. No schema or runtime changes are involved. Rolled out the portal binary with rollback copy `customer-portal.pre-detail-20260923`. Capacity admission, client-scoped roles and the other launch-plan features remain unfinished.

## September 23: website capacity admission and visible allowances

The portal now checks global and Node-specific project capacity inside the same SQLite transaction that inserts a project. A persisted project is its slot reservation, including unassigned and deleting projects. The existing cleanup controller releases it only when the final project row is removed after external cleanup. Rejected admission creates no project. Existing workspace subscription allowances still apply separately.

GET /api/projects includes workspace usage, an allowance when configured, and per-kind availability without disclosing other workspace counts. The new-website form displays the explanation, disables unavailable types and disables creation when both types are unavailable. Opening the form refreshes availability; POST always rechecks regardless of stale browser state. Operator flags are configured before serving requests and default to disabled for compatibility. Every project-creating process must share the same limits; this is admission to the current configured fleet, not a guarantee of worker health, autoscaling or arbitrary multi-controller policy coordination.

Live inventory confirmed six project rows, six corresponding fleet assignments and zero orphan assignments. Deployed portal admission limits of six total projects and five Node projects, matching the existing total provisioner limit and builder maximum. No projects were removed or created on the live fleet. Systemd override: `/etc/systemd/system/customer-portal.service.d/capacity.conf`. Rollback requires removing that override as well as restoring `customer-portal.pre-capacity-admission-20260923`, because the old binary lacks these flags.

Focused checks cover concurrent final-slot admission, Node-specific limits, retaining slots during deletion, release after cleanup, tenant authorization, HTTP 409 with no extra record, and invalid operator configuration. Race-enabled capacity tests, hosting-limit checks, the 22 frontend checks and Linux build passed. A disposable one-slot browser instance showed available creation followed by full-capacity copy and disabled creation. Public portal assets and the existing custom-domain site returned HTTP 200 after rollout. Next: richer deployment diagnostics and remaining client management features; the overall launch goal remains active and incomplete.

## September 23: publication diagnostics and retry guidance

Fixed Node workflow presentation after a failed publication: a retained build now shows Publishing needs attention and Retry publishing, rather than reverting to Ready to publish. The retry uses the saved release; it does not request a redundant build. Current active release still determines whether the same version is already live.

Node deployment summaries now provide safe queued/preparing/submitted-and-awaiting-verification/succeeded/failed/cancelled explanations. Preparation vs submission uses the persisted dispatch-intent presence only, not raw evidence. No runtime output, secrets or internal intent is returned. Deployment history adds request times and the explanation. Pending build/publication panels include the request time, support reference and a notice after five minutes that the operation may still be running. A timer updates that notice even when server status stays unchanged; elapsed time never changes server state or triggers a duplicate operation.

Validation: 24 frontend checks including failed-publication recovery choice and persisted-stage presentation, focused Go deployment progress/HTTP/recovery checks, and Linux build passed. This delivers customer diagnostics, not raw build or application logs. Rolled out with rollback binary `customer-portal.pre-deployment-status-20260923`; existing capacity flags remain supported. No user build, publication or restoration was initiated.

## September 23: failed-build output in the customer portal

The container builder now captures failed-container output before cleanup, retains a bounded sanitized tail in its private execution record, and carries it through the authenticated execution observation into the existing build result. The private observation field is optional for older records/providers. Updated all three active build workers before the builder because older strict protocol consumers do not recognize a nonempty new field. Artifact headers and successful-build output are unchanged.

Owners/developers can open View build output in failed build history. The dedicated read endpoint authorizes current project write access and reads the project/build result in the same transaction, returns only sanitized output, and excludes viewer/foreign-workspace access. The browser uses textContent, never HTML. Output memory capture is capped at 16 KiB; persisted/displayed output at 1.5 KiB. Common credential lines, private-key blocks, ANSI/control characters and URL user credentials are removed. This is defense in depth, not proof that arbitrary secrets printed by customer code can be recognized; do not place secrets in uploads. Logs follow retained build records and project cleanup.

No historical output is reconstructed: previously cleaned builds and failures before executor submission show an explicit no-output message. Current scope is completed failed container builds, not live streaming, successful-build logs or runtime application logs.

Checks: redaction/UTF-8/bounded-tail tests; authenticated transport round-trip with output; HTTP owner/foreign/viewer isolation and raw-evidence exclusion; Linux failed-container capture test using a disposable fake Docker command, without creating containers or touching customer data. Linux/ARM64 builder and Linux/AMD64 workers/portal built successfully. Live queue had no pending builds before upgrade. Rolled out workers, builder and portal; services active and public portal/custom-domain HTTP checks passed. Previous binaries have `.pre-failure-logs-20260923` suffix. For rollback, restore builder before old protocol consumers. No live customer build or publication was initiated to demonstrate the feature.

## September 23: on-demand runtime log snapshots

Added a private Unix-socket log bridge that reloads the operator fleet map, resolves only a valid assigned project ID to its existing `site-<prefix>` app, and calls the deployer's read-only logs command with a fixed 100-line request. It does not accept app names, commands, paths, or credentials from browsers. Concurrency is limited to two bridge reads; command/client output is bounded, sanitized and reduced to the existing 1.5 KiB display tail. The service socket lives in a 0700 systemd RuntimeDirectory and is 0600; no public listener was created.

The customer runtime-log endpoint checks current owner/developer project access both before and after the bridge call. Viewers, foreign workspaces, revoked membership and deleting projects cannot retrieve logs. Logs are fetched only when the user asks, not during background status polling, and are rendered as plain text with a fetch timestamp. The UI preserves the log section through project refreshes. Snapshots are not stored in the portal database; common credential redaction remains defense in depth, not a guarantee for arbitrary secrets printed by customer code.

Focused race-enabled tests cover private-socket round trip, malformed project rejection, redaction, workspace isolation, viewer denial and permission revocation during a read. Existing 24 frontend tests and Linux builds passed. The first bridge probe returned a temporary unavailable response; after restarting the service, a read of the existing Node website succeeded through the deployed bridge. No logs were printed into the task, and no customer app was restarted. Portal assets and custom-domain HTTP checks passed.

Deployed `launchstead-fleet-logs.service`, binary `/opt/launchstead-fleet/fleet-logs`, socket `/run/launchstead-logs/logs.sock`. Portal startup now includes `--runtime-log-socket` in its existing capacity override. Rollback requires restoring that override from `capacity.conf.pre-runtime-logs` before restoring `customer-portal.pre-runtime-logs-20260923`; the private bridge can then be stopped/disabled. Next substantial launch work includes client-scoped collaboration and Docker/GitHub deployment paths. All four original commercial/MVP requirements remain in scope and incomplete.
