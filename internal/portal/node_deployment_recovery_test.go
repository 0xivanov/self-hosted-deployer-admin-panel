...
func TestNodeDeploymentRecoveryMigratesPendingOperation(t *testing.T) {
	...
	if _, err = s.db.Exec("DROP TABLE node_active_deployments; ALTER TABLE node_deployments DROP COLUMN result; ALTER TABLE node_deployments DROP COLUMN dispatch_intent; DROP TABLE merchant_orders; DROP TABLE merchant_products; DROP TABLE merchant_accounts; PRAGMA user_version=22"); err != nil {
		t.Fatal(err)
	}
	if err = s.Close(); err != nil {
		t.Fatal(err)
