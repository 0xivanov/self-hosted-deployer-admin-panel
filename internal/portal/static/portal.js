...
let csrf='',workspaces=[],generation=0,flow='',billingEnabled=false,billingMode='test',billingManagement=false,inviteOnly=false,clientInvitations=false,githubConnections=false,githubImports=false,githubInstallation=false,githubAutoDeploy=false,billingGeneration=0,merchantEnabled=false,merchantMode='test',merchantCountries=[],merchantGeneration=0,productGeneration=0,merchantOrdersGeneration=0,domainQuotes=false,domainCheckout=false,domainOrderGeneration=0,domainPurchaseGeneration=0,domainExpiryTimer,domainPurchasePollTimer=null,domainPurchasePollCount=0,selectedProjectForDomain='';
...
function configureWorkspaceNavigation(selected){
 ...
 $('domain-panel').hidden=!(owner&&(domainQuotes||domainCheckout));
 for(const [view,available] of Object.entries({projects:true,shared:true,billing:owner&&billingEnabled,team:owner,domains:owner&&(domainQuotes||domainCheckout),store:owner&&merchantEnabled}))$('nav-'+view).hidden=!available;
...
function refreshCurrentBilling(){
 ...
 if(document.hidden||$('workspace-view').hidden||currentView!=='billing'||!billingEnabled||workspaces.find(w=>w.id===workspace)?.role!=='owner')return;
...
function refreshOpenGithubImports(){if(document.hidden)return;for(const box of document.querySelectorAll('.github-imports,.github-deployments'))box._githubRefresh?.();}
window.addEventListener('focus',refreshOpenGithubImports);
document.addEventListener('visibilitychange',refreshOpenGithubImports);

const fragment=new URLSearchParams(location.hash.slice(1));
const githubCallbackParams=new URLSearchParams(location.search);
const domainCheckoutReturn=githubCallbackParams.get('domain_checkout')==='return';
...
function restoreProjectCreateForm(){const create=$('new-project-details');if(create&&create.parentElement!==$('projects-panel'))$('projects-panel').insertBefore(create,$('portfolio-tools'));}
function signedOut(){restoreProjectCreateForm();sharedRequest++;$('shared-websites').replaceChildren();$('shared-status').textContent='';selectedWebsite='';stopStaticStatusRefresh();stopBillingRefresh();currentView='projects';stopNodeStatusRefresh();stopContainerStatusRefresh();generation++;$('error').hidden=true;resetDomainPanel();$('domain-panel').hidden=true;billingGeneration++;merchantGeneration++;productGeneration++;merchantOrdersGeneration++;$('billing-content').replaceChildren();$('billing-panel').hidden=true;$('merchant-content').replaceChildren();$('product-content').replaceChildren();$('merchant-orders-content').replaceChildren();$('merchant-panel').hidden=true;csrf='';workspaces=[];$('workspace-view').hidden=true;$('logout').hidden=true;$('login').hidden=false;$('account-flow').hidden=true;$('projects').replaceChildren();}
async function loadProjects(){projectAvailability=null;$('project-form').querySelector('button[type=submit],button').disabled=true;stopStaticStatusRefresh();stopBillingRefresh();billingPollCount=0;stopNodeStatusRefresh();const version=++generation;const workspace=$('workspace').value;const selected=workspaces.find(w=>w.id===workspace);resetDomainPanel();$('domain-panel').hidden=!domainQuotes||!selected||selected.role!=='owner';billingGeneration++;merchantGeneration++;productGeneration++;merchantOrdersGeneration++;$('billing-panel').hidden=!billingEnabled||!selected||selected.role!=='owner';$('billing-content').replaceChildren();$('merchant-panel').hidden=!merchantEnabled||!selected||selected.role!=='owner';$('merchant-content').replaceChildren();$('product-content').replaceChildren();$('merchant-orders-content').replaceChildren();$('project-form').hidden=!selected||selected.role==='viewer'; $('member-panel').hidden=!selected||selected.role!=='owner';$('members').replaceChildren();const create=$('new-project-details');if(create.parentElement!==$('projects-panel'))$('projects-panel').insertBefore(create,$('portfolio-tools'));$('projects').replaceChildren();$('projects').classList.remove('is-empty');configureWorkspaceNavigation(selected);if(!workspace)return;const data=await api('/api/projects?workspace='+encodeURIComponent(workspace));if(version!==generation)return;renderProjectAvailability(data.availability);if(selectedWebsite&&!data.projects.some(project=>project.id===selectedWebsite))selectedWebsite='';for(const project of data.projects){await appendProject(project,selected.role,version);if(version!==generation)return;}if(!data.projects.length)renderWorkspaceEmptyState(selected?.role);if(selected.role==='owner'){if(domainQuotes)loadDomainOrders(workspace,version);await loadMembers(workspace,version);if(version===generation&&billingEnabled)await loadBilling(workspace,version);if(version===generation&&merchantEnabled){await loadMerchant(workspace,version);loadMerchantProducts(workspace,version);loadMerchantOrders(workspace,version);}}if(version===generation)startNodeStatusRefresh(version,workspace);}
async function appendProject(project,role,version){if(version!==generation)return;$('projects').classList.remove('is-empty');const card=document.createElement('div');card.className='project';card.dataset.projectId=project.id;card.dataset.projectKind=project.kind;const name=document.createElement('strong');name.className='project-name';name.textContent=project.name;const kind=document.createElement('span');kind.textContent=(project.kind==='node'?'Node.js':project.kind==='container'?'Docker container':'Static website');const clientLabel=document.createElement('span');clientLabel.className='project-client-label';clientLabel.textContent=typeof project.client_label==='string'?project.client_label:'';clientLabel.hidden=!clientLabel.textContent;const heading=document.createElement('div');heading.className='project-heading';const icon=document.createElement('span');icon.className='project-icon';icon.textContent=project.kind==='node'?'JS':project.kind==='container'?'IMG':'</>';const headingInfo=document.createElement('div');headingInfo.className='project-heading-info';headingInfo.append(name,kind,clientLabel);heading.append(icon,headingInfo);card.append(heading);$('projects').append(card);if(project.deleting){renderDeletingProject(card,project,version);startProjectDeletionPolling(version,$('workspace').value);return;}await projectUploads(card,project,role,version);if(version!==generation||!card.isConnected)return;organizeProject(card);await renderProjectDomains(card,project,role,version);if(version!==generation||!card.isConnected)return;renderProjectGithub(card,project,role,version);renderProjectRename(card,project,role,version);renderProjectDeletion(card,project,role,version);renderRuntimeLogs(card,project,role,version);renderProjectClients(card,project,role,version);}
...
$('project-form').addEventListener('submit',event=>{event.preventDefault();submit(event.currentTarget,async()=>{const findDomain=$('project-domain-checkout').checked;const workspace=$('workspace').value;const created=await api('/api/projects',{workspace,name:$('project-name').value,kind:$('project-kind').value});$('project-name').value='';$('project-domain-checkout').checked=false;$('new-project-details').open=false;await loadProjects();if(findDomain&&workspace===$('workspace').value&&created?.id){selectedProjectForDomain=created.id;const selected=$('domain-selected-project');selected.textContent='Finding a domain for '+(created.name||'this website')+'.';selected.hidden=false;selectWorkspaceView('domains');loadDomainOrders(workspace,generation);$('domain-name').focus({preventScroll:true});}});});
...
async function initialize(){
 ...
 try{await loadSession();if(domainCheckoutReturn&&domainCheckout){selectWorkspaceView('domains');loadDomainOrders($('workspace').value,generation);}}catch(e){signedOut();if(e.status!==401)throw e;return;}
...
function renderProjectRename(card,project,role,version){
 ...
 const details=disclosure('Website settings','project-rename');const hint=document.createElement('p');hint.textContent='Update the name and add an internal client label for workspace organization. Leave the label blank to remove it. Labels never change access or publishing.';details.append(hint);
 ...
 const labelForm=document.createElement('form');labelForm.className='project-client-label-form';const labelField=document.createElement('label');labelField.textContent='Client label';const labelInput=document.createElement('input');labelInput.type='text';labelInput.maxLength=100;labelInput.autocomplete='off';labelInput.spellcheck=false;labelInput.placeholder='e.g. Acme · Marketing';labelInput.value=typeof project.client_label==='string'?project.client_label:'';labelField.append(labelInput);const labelButton=document.createElement('button');labelButton.type='submit';labelButton.textContent='Save client label';labelForm.append(labelField,labelButton);details.append(labelForm);labelForm.addEventListener('submit',event=>{event.preventDefault();submit(labelForm,async()=>{const value=labelInput.value.trim();const updated=await api('/api/projects/client-label',{project:project.id,label:value});if(version!==generation||!card.isConnected||card.dataset.deleting==='true')return;if(!updated||updated.id!==project.id||typeof updated.client_label!=='string')throw new Error('Client label response was invalid.');const next=updated.client_label;project.client_label=next;const labelNode=card.querySelector('.project-client-label');if(labelNode){labelNode.textContent=next;labelNode.hidden=!next;}else if(next){const node=document.createElement('span');node.className='project-client-label';node.textContent=next;card.querySelector('.project-heading-info')?.append(node);}labelInput.value=next;details.open=false;});});card.append(details);
}
...
function renderWorkspaceEmptyState(role){
 const projects=$('projects');if(projects.querySelector('.project'))return;restoreProjectCreateForm();projects.replaceChildren();projects.classList.add('is-empty');
 const panel=document.createElement('div');panel.className='workspace-empty-card';const title=document.createElement('h3');title.textContent=role==='viewer'?'No websites to show yet':'Start with a website';const copy=document.createElement('p');copy.textContent=role==='viewer'?'Websites shared with this workspace will appear here.':'Create a project for a client website. You can upload files, publish a version, and connect a domain from its workspace.';panel.append(title,copy);
 const create=$('new-project-details');if(role!=='viewer'){create.open=true;panel.append(create);}projects.append(panel);
}
function updateProjectsEmptyState(){renderWorkspaceEmptyState(workspaces.find(item=>item.id===$('workspace').value)?.role);}
...
function githubAccessGuide(container,project,details,status,pending=false){
 if(!githubInstallation)return;
 const guide=document.createElement('div');guide.className='github-access-guide';
 const title=document.createElement('strong');title.textContent='Grant access to your repositories';
 const copy=document.createElement('p');copy.textContent='Open GitHub in a new tab, choose your account or organization, and select only the repositories you want to host. Return here when you have saved access.';
 const action=document.createElement('div');action.textContent='Loading GitHub App setup…';guide.append(title,copy,action);container.append(guide);
 const load=async()=>{try{const data=await api('/api/github/installation?project='+encodeURIComponent(project.id));if(!guide.isConnected)return;if(typeof data.url!=='string'||!/^https:\/\/github\.com\/apps\/[a-z0-9-]{1,100}\/installations\/new$/.test(data.url))throw new Error('GitHub App setup could not be verified.');const link=document.createElement('a');link.href=data.url;link.target='_blank';link.rel='noopener noreferrer';link.textContent='Choose repositories on GitHub';action.replaceChildren(link);if(pending){const refresh=document.createElement('button');refresh.type='button';refresh.textContent='I saved access, refresh repositories';refresh.addEventListener('click',()=>renderGithubContent(details.querySelector('.github-content'),project,details,status));action.append(refresh);}}catch(e){if(!guide.isConnected)return;const message=document.createElement('p');message.textContent=e.message;const retry=document.createElement('button');retry.type='button';retry.textContent='Retry GitHub App setup';retry.addEventListener('click',()=>{retry.disabled=true;load();});action.replaceChildren(message,retry);}};load();
}
function githubRenderChooser(content,project,details,status){
 ...
 api('/api/github/repositories?project='+encodeURIComponent(project.id)).then(data=>{if(!chooser.isConnected)return;chooser.replaceChildren();const repos=Array.isArray(data.repositories)?data.repositories:[];if(!repos.length){const empty=document.createElement('p');empty.className='muted';empty.textContent='No repositories are available. Install or manage the Launchstead GitHub App, then restart the connection.';chooser.append(empty);githubAccessGuide(chooser,project,details,status,true);const restart=githubStartButton(project,details,status,chooser);restart.textContent='Restart GitHub connection';return;}
  const form=document.createElement('form');const repoLabel=document.createElement('label');repoLabel.textContent='Repository';const repoSelect=document.createElement('select');repoSelect.required=true;repos.forEach(repo=>{const installation=Number(repo.InstallationID??repo.installation_id??repo.installationId);const repository=Number(repo.RepositoryID??repo.repository_id??repo.repositoryId);if(!Number.isSafeInteger(installation)||installation<=0||!Number.isSafeInteger(repository)||repository<=0)return;const option=document.createElement('option');option.value=String(repository);option.dataset.installation=String(installation);option.dataset.branch=String(repo.DefaultBranch??repo.default_branch??repo.defaultBranch??'main');option.textContent=repo.RepositoryFullName||repo.repository_full_name||repo.repository||'Repository';repoSelect.append(option);});repoLabel.append(repoSelect);const branchLabel=document.createElement('label');branchLabel.textContent='Branch';const branch=document.createElement('input');branch.required=true;branch.value=repoSelect.selectedOptions[0]?.dataset.branch||'main';branchLabel.append(branch);const directoryLabel=document.createElement('label');directoryLabel.textContent='Directory (optional)';const directory=document.createElement('input');directory.placeholder='/';directoryLabel.append(directory);const connect=document.createElement('button');connect.type='submit';connect.textContent='Connect repository';form.append(repoLabel,branchLabel,directoryLabel,connect);repoSelect.addEventListener('change',()=>{branch.value=repoSelect.selectedOptions[0]?.dataset.branch||'main';});form.addEventListener('submit',async event=>{event.preventDefault();connect.disabled=true;status.textContent='Saving GitHub connection…';const selected=repoSelect.selectedOptions[0];try{if(!selected)throw new Error('No valid GitHub repositories are available.');await api('/api/github/connect',{project:project.id,installation_id:Number(selected.dataset.installation),repository_id:Number(selected.value),branch:branch.value.trim(),directory:directory.value.trim()});if(chooser.isConnected)await renderGithubContent(content,project,details,status);}catch(e){status.textContent=e.message;connect.disabled=false;}});chooser.append(form);githubAccessGuide(chooser,project,details,status,true);
 }).catch(e=>{if(!chooser.isConnected)return;chooser.replaceChildren();const message=document.createElement('p');message.className='muted';message.textContent=e.message+' Install or manage the Launchstead GitHub App, then restart the connection.';chooser.append(message);githubAccessGuide(chooser,project,details,status,true);const restart=githubStartButton(project,details,status,chooser);restart.textContent='Restart GitHub connection';});
}
const githubImportStates=new WeakMap();
function stopGithubImportPolling(details){const state=githubImportStates.get(details);if(state?.timer){clearTimeout(state.timer);state.timer=null;}}
function githubImportDate(value){if(!value)return '';const date=new Date(typeof value==='number'?value*1000:value);return Number.isNaN(date.getTime())?'':date.toLocaleString();}
function githubTimestamp(value){const number=Number(value);if(Number.isFinite(number))return number;const parsed=Date.parse(value||'');return Number.isFinite(parsed)?parsed:0;}
function githubImportError(error){const message=String(error?.message||'');if(/source_unavailable/i.test(message))return 'The connected repository source is unavailable. Check the GitHub App access and retry.';if(/archive_invalid/i.test(message))return 'The selected source does not meet this project’s upload requirements. Check the folder, entry files and archive size before retrying.';if(/import_unavailable/i.test(message))return 'The import could not be saved. Check your hosting plan, upload limits and repository connection before retrying.';return message;}
function renderGithubImports(content,project,role,version,details){
 if(!githubImports||role!=='owner')return;
 const existing=content.querySelector('.github-imports');if(existing)return;
 const box=document.createElement('div');box.className='github-imports';const heading=document.createElement('strong');heading.textContent='Import source';const note=document.createElement('p');note.className='muted';note.textContent='Imports the latest repository commit into your existing uploads. Build and publish it through the normal workflow; imports do not auto-deploy.';const status=document.createElement('p');status.className='github-import-status muted';const button=document.createElement('button');button.type='button';button.textContent='Import latest commit';box.append(heading,note,status,button);content.append(box);
 const state=githubImportStates.get(details)||{requestKey:'',job:null,timer:null,inFlight:false,pollAttempts:0,refreshedJob:''};githubImportStates.set(details,state);const current=()=>box.isConnected&&details.isConnected&&details.open&&!document.hidden&&currentView==='projects'&&version===generation&&state===githubImportStates.get(details);
 function schedule(){stopGithubImportPolling(details);if(!current())return;if(state.pollAttempts<12){state.pollAttempts++;state.timer=setTimeout(load,5000);}else status.textContent='Import is still running. Reopen this section to check again.';}
 function draw(job){if(!job){status.textContent='No imports yet.';button.disabled=false;button.textContent='Import latest commit';return;}state.job=job;const stateName=String(job.state||'').toLowerCase();status.textContent=stateName==='succeeded'?'Imported commit '+(job.commit||'latest')+' at '+githubImportDate(job.created_at):stateName==='failed'?'Import failed: '+githubImportError({message:job.error||'Try again with a fresh import request.'}):stateName==='queued'||stateName==='running'?'Import '+stateName+'…':'Import status: '+(job.state||'unknown');button.disabled=stateName==='queued'||stateName==='running'||state.inFlight;button.textContent=stateName==='failed'?'Retry failed import':stateName==='queued'||stateName==='running'?'Import in progress':'Import latest commit';state.statusRetry=false;if(stateName==='failed')state.requestKey='';if(stateName==='succeeded'){state.requestKey='';if(state.refreshedJob!==job.id&&state.refreshingJob!==job.id){state.refreshingJob=job.id;refreshProject(project,role,version).then(()=>{state.refreshedJob=job.id;state.refreshingJob='';}).catch(()=>{state.refreshingJob='';status.textContent='Import succeeded, but the upload list could not be refreshed. Reopen this section to retry.';});}}if(stateName==='queued'||stateName==='running')schedule();}
 async function load(){if(!current())return;const request=state.statusRequest=(state.statusRequest||0)+1;try{const data=await api('/api/github/imports?project='+encodeURIComponent(project.id));if(!current()||request!==state.statusRequest)return;const imports=Array.isArray(data.imports)?data.imports:[];imports.sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||'')));draw(imports[0]||null);}catch(e){if(current()&&request===state.statusRequest){status.textContent=githubImportError(e);state.statusRetry=true;button.disabled=false;button.textContent='Retry import status';}}}
 button.addEventListener('click',async()=>{if(state.statusRetry){state.statusRetry=false;await load();return;}if(state.inFlight||!current())return;state.inFlight=true;if(!state.requestKey)state.requestKey=crypto.randomUUID();button.disabled=true;status.textContent='Starting import…';try{const data=await api('/api/github/imports',{project:project.id,request_key:state.requestKey});state.inFlight=false;if(!current())return;state.pollAttempts=0;draw(data.import||null);}catch(e){state.inFlight=false;if(current()){status.textContent=(githubImportError(e)||'Import request did not complete.')+' Retry to resume the same request.';button.disabled=false;button.textContent='Retry import';}}});
 box._githubRefresh=()=>{if(details.open)load();};if(state.toggleHandler)details.removeEventListener('toggle',state.toggleHandler);state.toggleHandler=()=>{if(details.open){state.pollAttempts=0;load();}else stopGithubImportPolling(details);};details.addEventListener('toggle',state.toggleHandler);load();
}
const githubDeploymentStates=new WeakMap();
function githubDeploymentError(value){const message=typeof value==='string'?value:'';const errors={source_unavailable:'The repository source is unavailable. Check GitHub App access.',connection_changed:'The GitHub connection changed while this deployment was running. Use the normal upload and build controls to recover.',hosting_unavailable:'Hosting is temporarily unavailable. Use the normal upload and build controls to recover.',deployment_unavailable:'Automatic deployment is temporarily unavailable. Use the normal upload and build controls to recover.',build_failed:'The build failed. Use the normal upload and build controls to recover.',publication_failed:'Publishing failed. Use the normal upload and build controls to recover.'};return errors[message.toLowerCase()]||'Automatic deployment failed. Use the normal upload and build controls to recover.';}
function githubDeploymentLabel(value){const labels={waiting:'Waiting',building:'Building',publishing:'Publishing',succeeded:'Succeeded',failed:'Failed',cancelled:'Cancelled',superseded:'Superseded'};return labels[String(value||'').toLowerCase()]||'Status unavailable';}
function renderGithubDeployments(content,project,role,version,details,enabled){
 if(!githubAutoDeploy||role!=='owner')return;
 const existing=content.querySelector('.github-deployments');if(existing)return;
 const box=document.createElement('div');box.className='github-deployments';const heading=document.createElement('strong');heading.textContent='Automatic deployment activity';const status=document.createElement('p');status.className='github-deployment-status muted';const list=document.createElement('div');list.className='github-deployment-list';box.append(heading,status,list);content.append(box);
 const state=githubDeploymentStates.get(details)||{timer:null,attempts:0,request:0};githubDeploymentStates.set(details,state);const current=()=>box.isConnected&&details.isConnected&&details.open&&!document.hidden&&currentView==='projects'&&version===generation&&state===githubDeploymentStates.get(details);
 function schedule(){if(state.timer)clearTimeout(state.timer);if(current()&&state.attempts<12){state.attempts++;state.timer=setTimeout(load,5000);}else if(current())status.textContent='Deployment activity is still updating. Reopen this section to check again.';}
 function draw(deployments){list.replaceChildren();if(!deployments.length){status.textContent='No automatic deployments yet.';if(enabled)schedule();return;}status.textContent='';deployments.slice(0,8).forEach(deployment=>{const row=document.createElement('div');row.className='github-deployment-row';const stateText=document.createElement('strong');stateText.textContent=githubDeploymentLabel(deployment.state);const commit=document.createElement('span');commit.textContent=deployment.commit||'Unknown commit';const updated=document.createElement('small');updated.textContent=githubImportDate(deployment.updated_at);row.append(stateText,commit,updated);if(/failed/i.test(deployment.state||'')){const failure=document.createElement('p');failure.className='muted';failure.textContent=githubDeploymentError(deployment.error);row.append(failure);}list.append(row);});const active=deployments.some(deployment=>/^(waiting|building|publishing)$/i.test(deployment.state||''));if(active||enabled)schedule();}
 async function load(){if(!current())return;const request=++state.request;try{const data=await api('/api/github/deployments?project='+encodeURIComponent(project.id));if(!current()||request!==state.request)return;const deployments=Array.isArray(data.deployments)?data.deployments:[];deployments.sort((a,b)=>githubTimestamp(b.updated_at)-githubTimestamp(a.updated_at));draw(deployments);}catch(e){if(current()&&request===state.request)status.textContent=githubDeploymentError(e);}}
 box._githubRefresh=()=>{if(details.open){state.attempts=0;load();}};if(state.toggleHandler)details.removeEventListener('toggle',state.toggleHandler);state.toggleHandler=()=>{if(details.open){state.attempts=0;load();}else if(state.timer){clearTimeout(state.timer);state.timer=null;}};details.addEventListener('toggle',state.toggleHandler);load();
}
function renderGithubAutoDeploy(content,project,connection,details,status){
 if(!githubAutoDeploy)return;
 const wrapper=document.createElement('div');wrapper.className='github-auto-deploy';const label=document.createElement('label');const toggle=document.createElement('input');toggle.type='checkbox';toggle.checked=connection?.deploy_on_push===true;const copy=document.createElement('span');copy.textContent='Enable automatic deployments';label.append(toggle,copy);const note=document.createElement('p');note.className='muted';note.textContent='The next repository push will build and publish automatically. Disabling stops queued future work and keeps the live website online.';wrapper.append(label,note);content.append(wrapper);toggle.addEventListener('change',async()=>{const enabled=toggle.checked;toggle.disabled=true;status.textContent=enabled?'Enabling automatic deployments…':'Disabling automatic deployments…';try{await api('/api/github/auto-deploy',{project:project.id,enabled});if(content.isConnected)await renderGithubContent(content,project,details,status);}catch(e){toggle.checked=!enabled;toggle.disabled=false;status.textContent=e.message;}});
}
async function renderGithubContent(content,project,details,status,choose){
 ...
 try{const data=await api('/api/github/connection?project='+encodeURIComponent(project.id));if(!current())return;const connection=data.connection;const pending=data.pending===true;status.textContent='';
  if(connection?.connected===true){const heading=document.createElement('strong');heading.textContent='Repository connected';const repo=document.createElement('p');repo.className='github-repository';repo.textContent=githubRepositoryName(connection);const meta=document.createElement('p');meta.className='muted';meta.textContent='Branch: '+(githubConnectionValue(connection,'branch')||'default')+(githubConnectionValue(connection,'directory')?' · Directory: '+githubConnectionValue(connection,'directory'):'');const note=document.createElement('p');note.className='github-note';note.textContent=githubAutoDeploy?'Choose automatic deployments below, or import a commit and publish it manually.':githubImports?'Import source below, then use the usual Build or Publish controls. Automatic deployments on push are not enabled.':'Repository connected. GitHub publishing is not enabled yet; ZIP uploads still work.';const disconnect=document.createElement('button');disconnect.type='button';disconnect.className='button-danger';disconnect.textContent='Disconnect';disconnect.addEventListener('click',async()=>{if(!confirm('Disconnect this GitHub repository from '+project.name+'?'))return;disconnect.disabled=true;try{await api('/api/github/disconnect',{project:project.id});if(content.isConnected)await renderGithubContent(content,project,details,status);}catch(e){status.textContent=e.message;disconnect.disabled=false;}});content.append(heading,repo,meta,note);renderGithubAutoDeploy(content,project,connection,details,status);renderGithubImports(content,project,project.role||'owner',generation,details);renderGithubDeployments(content,project,project.role||'owner',generation,details,connection.deploy_on_push===true);githubAccessGuide(content,project,details,status);content.append(disconnect);status.textContent='';return;}
  ...
  const note=document.createElement('p');note.className='muted';note.textContent='Connect a repository to keep its source available alongside your ZIP uploads.';content.append(note);githubAccessGuide(content,project,details,status);const restart=githubStartButton(project,details,status,content);restart.textContent='Connect GitHub';
...
async function loadBilling(workspace,version){
 ...
 if(!returnMessage.hidden){
  ...
  returnMessage.textContent=active?'Your '+billingMode+' subscription is active.'+(billingMode==='test'?' No real money was charged.':''):location.pathname==='/billing/cancel'?'Checkout was canceled. You can continue when you are ready.':waiting?(billingPollCount<24?'Checking your subscription with Stripe. This page updates automatically.':'Stripe is taking longer to confirm this checkout. Use Refresh status to check again.'):'Your latest subscription status is shown below.';
 }
 ...
 if(access.mode==='test_subscription'||access.mode==='live_subscription'){const label=access.mode==='test_subscription'?'test subscription':'hosting subscription';note(access.allowed?'Hosting changes are enabled by your verified '+label+'.':'Hosting changes are on hold. A current paid '+label+' and payment check are required. Existing sites keep running.');}
 ...
 if(status.checkout){
  ...
  if(checkout.state==='open'&&checkout.url){const url=new URL(checkout.url);if(url.protocol==='https:'&&url.host==='checkout.stripe.com'&&!url.username&&!url.password){const link=document.createElement('a');link.href=url.href;link.target='_blank';link.rel='noopener noreferrer';link.className='primary-link';link.textContent='Continue to '+billingMode+' checkout';content.append(link);}}
  return;
 }
 if(status.customer_state==='not_started'){note('Set up billing for this workspace to choose a hosting plan.');action('Set up '+billingMode+' billing',()=>api('/api/billing/customer',{workspace}));return;}
...
$('billing-refresh').addEventListener('click',()=>{const workspace=$('workspace').value;if(billingEnabled&&workspaces.find(w=>w.id===workspace)?.role==='owner')loadBilling(workspace,generation).catch(error);});


function resetDomainPanel(){
 domainOrderGeneration++;domainPurchaseGeneration++;clearTimeout(domainPurchasePollTimer);domainPurchasePollTimer=null;domainPurchasePollCount=0;selectedProjectForDomain='';$('domain-orders').replaceChildren();$('domain-purchases').replaceChildren();$('domain-selected-project').hidden=true;
...
function showDomainQuote(quote,version){
 ...
 note(offer.domain);
 if(offer.environment==='sandbox')note('Sandbox test quote. This does not represent real domain availability or authorize a purchase.');
 ...
 const prepare=document.createElement('button');prepare.type='button';prepare.textContent=offer.environment==='sandbox'?'Prepare test order':'Prepare domain order';let preparing=false;
...
async function loadDomainOrders(workspace,version){
 ...
 try{
  ...
  if(domainCheckout)loadDomainPurchases(workspace,version);
...
$('domain-orders-refresh').addEventListener('click',()=>{const workspace=$('workspace').value;if(domainCheckout&&workspaces.find(w=>w.id===workspace)?.role==='owner')loadDomainPurchases(workspace,generation);});
$('nav-domains').addEventListener('click',()=>{const workspace=$('workspace').value;if(domainCheckout&&workspaces.find(w=>w.id===workspace)?.role==='owner')loadDomainPurchases(workspace,generation);});

function purchaseStateLabel(state){return ({payment_pending:'Awaiting payment',registering:'Registering',connecting:'Connecting',connected:'Sandbox domain connected',needs_review:'Needs review'})[state]||'Status unavailable';}
function safeCheckoutURL(value){try{const url=new URL(value);return url.protocol==='https:'&&url.host==='checkout.stripe.com'&&!url.username&&!url.password?url.href:'';}catch{return '';}}
async function loadDomainPurchases(workspace,version){
 if(!domainCheckout||version!==generation||workspace!==$('workspace').value)return;
 const request=++domainPurchaseGeneration;const current=()=>version===generation&&request===domainPurchaseGeneration&&workspace===$('workspace').value;
 try{const data=await api('/api/domains/purchases?workspace='+encodeURIComponent(workspace));if(!current())return;const content=$('domain-purchases');content.replaceChildren();const purchases=Array.isArray(data.purchases)?data.purchases:[];if(!purchases.length){content.textContent='No domain purchases yet.';return;}let pending=false;
  for(const purchase of purchases){const row=document.createElement('div');row.className='project';const title=document.createElement('strong');title.textContent=domainText(purchase.domain)||'Sandbox domain';row.append(title);const detail=document.createElement('p');detail.textContent=purchase.message||purchaseStateLabel(purchase.state);row.append(detail);const state=document.createElement('span');state.className='domain-purchase-state domain-purchase-state-'+domainText(purchase.state);state.textContent=purchaseStateLabel(purchase.state);row.append(state);if(['payment_pending','registering','connecting'].includes(purchase.state))pending=true;if(purchase.state==='payment_pending'&&safeCheckoutURL(purchase.checkout_url)){const pay=document.createElement('a');pay.href=safeCheckoutURL(purchase.checkout_url);pay.target='_blank';pay.rel='noopener noreferrer';pay.className='primary-link';pay.textContent='Pay with Stripe test checkout';row.append(pay);}if(purchase.state==='payment_pending'||purchase.state==='needs_review'){const sync=document.createElement('button');sync.type='button';sync.textContent='Sync purchase';sync.addEventListener('click',async()=>{sync.disabled=true;try{await api('/api/domains/sync',{workspace,id:purchase.order_id});await loadDomainPurchases(workspace,version);}catch(e){error(e);sync.disabled=false;}});row.append(sync);}content.append(row);}
  if(pending&&domainPurchasePollCount<12){clearTimeout(domainPurchasePollTimer);domainPurchasePollTimer=setTimeout(()=>{domainPurchasePollCount++;loadDomainPurchases(workspace,version);},5000);}
 }catch(e){if(current()){$('domain-purchases').textContent='Domain purchase status is unavailable.';error(e);}}
}
...
function updateWebsiteSummary(card){
 ...
 if(!links){
  links=document.createElement('div');links.className='website-detail-links';
