...
let csrf='',workspaces=[],generation=0,flow='',testBilling=false,billingManagement=false,inviteOnly=false,clientInvitations=false,githubConnections=false,githubImports=false,githubInstallation=false,billingGeneration=0,merchantEnabled=false,merchantCountries=[],merchantGeneration=0,productGeneration=0,merchantOrdersGeneration=0,domainQuotes=false,domainOrderGeneration=0,domainExpiryTimer;
...
function refreshOpenGithubImports(){if(document.hidden)return;for(const box of document.querySelectorAll('.github-imports'))box._githubRefresh?.();}
