...
async function projectUploads(card,project,role,version){
 ...
 const status=document.createElement('p');status.textContent=active?'Published revision '+active.revision:publication.available?'Ready to publish':'Publishing awaits runtime assignment';card.append(status);
 ...
 for(const upload of result.uploads){const row=document.createElement('div');const text=document.createElement('p');text.textContent='Validated · '+upload.files+' files · '+(upload.compressed_bytes/1024).toFixed(1)+' KiB · '+new Date(upload.created_at*1000).toLocaleString();row.append(text);
 if(publication.available&&role!=='viewer'){const publish=document.createElement('button');const previous=publication.jobs.some(j=>j.upload_id===upload.id&&j.state==='succeeded');publish.textContent=previous?'Restore this upload':'Publish this upload';publish.disabled=pending;const requestKey=crypto.randomUUID();publish.addEventListener('click',async()=>{publish.disabled=true;try{await api('/api/publications',{project:project.id,upload:upload.id,key:requestKey});if(version===generation)await loadProjects();}catch(e){error(e);publish.disabled=false;}});row.append(publish);}
