...
func TestMerchantRefundEventMigrationKeepsCheckoutReceipts(t *testing.T) {
	...
	if _, err := s.db.Exec("ALTER TABLE merchant_events DROP COLUMN refund_request_id; ALTER TABLE merchant_events DROP COLUMN provider_refund_id; PRAGMA user_version=30"); err != nil {
		t.Fatal(err)
	}
	if err := s.Close(); err != nil {
		t.Fatal(err)
