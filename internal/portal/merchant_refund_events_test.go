...
func TestMerchantRefundEventMigrationKeepsCheckoutReceipts(t *testing.T) {
	...
	if _, err := s.db.Exec("ALTER TABLE merchant_events DROP COLUMN refund_request_id; ALTER TABLE merchant_events DROP COLUMN provider_refund_id; ALTER TABLE merchant_orders DROP COLUMN fulfilled_at; ALTER TABLE merchant_orders DROP COLUMN fulfilled_by; DROP TABLE hosting_workspace_policies; DROP TABLE merchant_order_recovery_grants; DROP TABLE merchant_order_recovery_codes; DROP TABLE merchant_buyer_sessions; DROP TABLE domain_orders; PRAGMA user_version=30"); err != nil {
		t.Fatal(err)
	}
	if err := s.Close(); err != nil {
		t.Fatal(err)
