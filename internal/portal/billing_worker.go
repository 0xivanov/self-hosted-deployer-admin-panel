...
func (s *Store) performBillingWork(ctx context.Context, p BillingProvider, kind, reference string) error {
	...
	case "event":
		...
		if err == nil && !done {
			reader, ok := p.(BillingChargeReader)
			if !ok {
				return ErrInvalid
			}
			done, err = s.ProcessBillingChargeEvent(ctx, reference, reader)
		}
		if err != nil {
			return err
		}
