...
func TestNodeDispatchMigrationPreservesPreparedBuild(t *testing.T) {
	...
	if _, err := s.db.Exec("DROP TABLE node_active_deployments; DROP TABLE node_deployment_releases; DROP TABLE node_deployments; DROP TABLE node_releases; ALTER TABLE node_builds DROP COLUMN dispatch_intent; DROP TABLE merchant_events; DROP TABLE merchant_orders; DROP TABLE merchant_products; DROP TABLE merchant_accounts; PRAGMA user_version=19"); err != nil {
		t.Fatal(err)
	}
	if err := s.Close(); err != nil {
		t.Fatal(err)
