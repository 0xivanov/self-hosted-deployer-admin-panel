...
func TestMerchantBuyerLegacyMigration(t *testing.T) {
	...
	if _, err := s.db.Exec("DROP TABLE merchant_order_recovery_grants; DROP TABLE merchant_order_recovery_codes; DROP TABLE merchant_buyer_sessions; PRAGMA user_version=33"); err != nil {
		t.Fatal(err)
	}
