...
func TestNodeReleaseRetentionCountAndMigration(t *testing.T) {
	...
	if _, err = s.db.Exec("DROP TABLE node_active_deployments; DROP TABLE node_deployment_releases; DROP TABLE node_deployments; DROP TABLE node_releases; DROP TABLE merchant_orders; DROP TABLE merchant_products; DROP TABLE merchant_accounts; PRAGMA user_version=20"); err != nil {
		t.Fatal(err)
	}
	if err = s.Close(); err != nil {
		t.Fatal(err)
