...
func (s *Store) FulfillMerchantOrder(ctx context.Context, token, workspace, id string) (MerchantOrder, error) {
	...
	order, _, _, _, err := scanMerchantOrder(tx.QueryRowContext(ctx, "SELECT "+merchantOrderColumns+" FROM merchant_orders WHERE id=? AND workspace_id=?", id, workspace))
	...
	var blocked int
	if err = tx.QueryRowContext(ctx, "SELECT count(*) FROM merchant_refunds WHERE order_id=? AND state NOT IN ('failed','canceled')", id).Scan(&blocked); err != nil {
		return MerchantOrder{}, err
	}
	if blocked != 0 {
		return MerchantOrder{}, ErrBillingConflict
	...
	if _, err = tx.ExecContext(ctx, "UPDATE merchant_orders SET fulfilled_at=?,fulfilled_by=? WHERE id=?", now, actor, id); err != nil {
		return MerchantOrder{}, err
	}
