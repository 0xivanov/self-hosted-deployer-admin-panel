...
func TestNodeRuntimeDispatchMigrationFencesLegacyRunning(t *testing.T) {
	t.Parallel()
	s, path, _, c := dispatchDeploymentFixture(t)
	if _, err := s.db.Exec("ALTER TABLE node_deployments DROP COLUMN dispatch_intent; DROP TABLE hosting_plan_limits; DROP TABLE hosting_workspace_policies; DROP TABLE merchant_order_recovery_grants; DROP TABLE merchant_order_recovery_codes; DROP TABLE merchant_buyer_sessions; DROP TABLE domain_orders; DROP TABLE merchant_refunds; DROP TABLE merchant_events; DROP TABLE merchant_orders; DROP TABLE merchant_products; DROP TABLE merchant_accounts; PRAGMA user_version=23"); err != nil {
		t.Fatal(err)
	}
	if err := s.Close(); err != nil {
		t.Fatal(err)
	...
	var version int
	if err = s.db.QueryRow("PRAGMA user_version").Scan(&version); err != nil || version != 37 {
		t.Fatal(version, err)
	}
}
