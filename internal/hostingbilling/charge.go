...
func (c *Client) RetrieveChargeObservation(ctx context.Context, id string) (ChargeObservation, error) {
	...
	if charge == nil || charge.ID != id || charge.Object != "charge" || charge.Livemode != c.live || charge.Customer == nil || !providerID(charge.Customer.ID, "cus_") || charge.PaymentIntent == nil || !providerID(charge.PaymentIntent.ID, "pi_") || charge.Application != nil || charge.ApplicationFee != nil || charge.OnBehalfOf != nil || charge.SourceTransfer != nil || charge.TransferData != nil || !charge.Paid || !charge.Captured || charge.Status != "succeeded" || charge.AmountCaptured <= 0 || charge.AmountCaptured > charge.Amount || charge.AmountRefunded < 0 || charge.AmountRefunded > charge.AmountCaptured {
...
func (c *Client) retrieveChargeDisputes(ctx context.Context, charge *stripe.Charge) ([]ChargeDispute, error) {
	...
	for _, d := range list.Data() {
		if d == nil || !providerID(d.ID, "dp_") || seen[d.ID] || d.Object != "dispute" || d.Livemode != c.live || d.Charge == nil || d.Charge.ID != charge.ID || d.PaymentIntent == nil || d.PaymentIntent.ID != charge.PaymentIntent.ID || d.Currency != charge.Currency || d.Amount <= 0 {
			return nil, invalid
		}
		switch d.Status {
