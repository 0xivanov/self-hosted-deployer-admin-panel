...
func run() error {
	...
	pin, e := hex.DecodeString(assignment.Toolchain)
	if e != nil || len(pin) != 32 || hex.EncodeToString(pin) != assignment.Toolchain || (assignment.Architecture != "arm64" && assignment.Architecture != "amd64") {
		return errors.New("invalid runtime toolchain or architecture")
	}
	...
	client, err := noderuntimeapi.NewClient(assignment.Endpoint, assignment.Project, assignment.Runtime, assignment.Token, roots)
	if err != nil {
		return err
	}
	defer client.Close()
	...
	for {
		...
		_, err := store.WorkNodeDeployment(ctx, assignment.Project, assignment.Runtime, assignment.Toolchain, assignment.Architecture, client)
		if ctx.Err() != nil {
			return nil
		}
		if err != nil {
			fmt.Fprintln(os.Stderr, "Node deployment needs runtime reconciliation; inspect assigned project history.")
		}
